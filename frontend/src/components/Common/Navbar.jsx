    ...
    <>
      <nav className="container mx-auto flex items-center justify-between py-4 px-6">
        ...
        <div className="hidden md:flex space-x-6">
          <Link to="/collections/all?gender=Men" className="text-shadow-gray-700 hover:text-black text-sm font-medium uppercase">
            Men
          </Link>
          <Link to="/collections/all?gender=Women" className="text-shadow-gray-700 hover:text-black text-sm font-medium uppercase">
            Women
          </Link>
          <Link to="/collections/all?category=Top Wear" className="text-shadow-gray-700 hover:text-black text-sm font-medium uppercase">
            Top Wear
          </Link>
          <Link to="/collections/all?category=Bottom Wear" className="text-shadow-gray-700 hover:text-black text-sm font-medium uppercase">
            Bottom Wear
          </Link>
        </div>
        {/* Right icons */}
        <div className="flex items-center space-x-4">
          ...
          <button onClick={toggleCartDrawer} className="relative hover:text-black hover:cursor-pointer">
          ...
          <button onClick={toggleNavDrawer} className="md:hidden hover:cursor-pointer">
      ...
      {/* Overlay */}
      <div
        className={`fixed inset-0 bg-black z-40 transition-opacity duration-300 ${navDrawerOpen ? "opacity-30 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
      ...
      <div className={`fixed top-0 left-0 w-3/4 sm:w-1/2 md:w-1/3 h-full bg-white shadow-lg transform transition-transform duration-300 z-50 ${navDrawerOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex justify-end p-4">
          <button>
            <IoMdClose onClick={toggleNavDrawer} className="h-6 w-6 text-gray-600 hover:cursor-pointer" />
          </button>
        </div>
        <div className="p-4">
